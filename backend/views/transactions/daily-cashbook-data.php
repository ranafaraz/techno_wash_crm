...
if(isset($_POST['sdate']) && isset($_POST['edate']))
{
		...
		$head_query = AccountHead::find()->where(['account_name'=>'Cash'])->One();
		if(isset($head_query))
		{
			$returndata.='
						<div class="row">
						...
						<div class="row" id="show-record">
							<div class="row">
								<div class="col-md-2">
									<img src="images/techno.png" height="80px" width="80px">
								</div>
			...
			$debit_transactions = Transactions::find()->where(['between', 'transactions_date', "$start_date", "$end_date"])->andwhere(["credit_account" => $head_query->id])->all();
			...
			foreach($debit_transactions as $val1)
			{
				...
				<td>'.$val1->transactions_date.'</td><td>'.$val1->type.'</td>
					...
					{
						$Bank = $bank + $val1->amount;
						$returndata.='<td>0</td>
						<td>'.number_format($val1->amount).'</td>
						</tr>';
					}
			}
			$returndata.='<tr class="text-primary"><th colspan="6" class="text-right">Total Payment:</th><th class="text-center">'.number_format($sum).'</th><th class="text-center">'.number_format($bank).'</th></tr></table></div>';
		}
	}
echo $returndata;
?>
